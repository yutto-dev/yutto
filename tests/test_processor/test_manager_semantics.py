...
async def test_execute_runs_requests_concurrently_and_preserves_result_order():
    ...
    assert [item.output_path for item in result.items] == [Path("BV1first"), Path("BV1second")]
...
@as_sync
async def test_concurrent_batch_reports_title_only_when_item_starts(monkeypatch: pytest.MonkeyPatch):
