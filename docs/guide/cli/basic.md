...
本参数限制同时处理的视频数量，适用于 URL 列表和批量来源中的多个分集。`--jobs` 控制视频级并发，`--num-workers` 控制每个视频内部的分块下载 worker；例如 `-j 3 -n 8` 最多会同时处理 3 个视频，每个视频最多使用 8 个下载 worker。

启用视频并发后，CLI 会在终端底部为正在下载的视频分别显示进度条。非交互终端或使用 `--no-progress` 时仍输出普通日志。
