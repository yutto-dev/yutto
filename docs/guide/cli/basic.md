...
- 可选参数变量 `title | id | name | username | series_title | pubdate | download_date | owner_uid | mp_title` （以后可能会有更多）
- 默认值 `"{auto}"`
...
| name | 系列视频单 p 标题 | 全部 |
| mp_title | 多 P 友好标题，由 `--mp-title-preset` 控制组合方式；单 P 时与 `title` 相同 | 全部 |
...
- 参数 `--mp-title-preset`
- 配置项 `basic.mp_title_preset`
- 默认值 `"title-dot-name"`

用于控制路径变量 `{mp_title}` 以及 NFO 元数据中的 `title` / `show_title` 在**多 P 视频**下的构成方式。单 P 视频始终使用总标题 `title`。

组合类预设会在分 P 标题包含总标题全文时自动去除重合部分。

| 取值 | 构成方式 | 示例（多 P 的 p2） |
| - | - | - |
| `title` | 总标题 | `【SING女团】……飒爽上线` |
| `name` | 分 P 标题 | `完整舞蹈版` |
| `title-dot-name`（默认） | `title.name` | `【SING女团】……飒爽上线.完整舞蹈版` |
| `title-dot-name-with-id` | `title.p{id}.name` | `【SING女团】……飒爽上线.p2.完整舞蹈版` |
| `title-hyphen-name` | `title-name` | `【SING女团】……飒爽上线-完整舞蹈版` |
| `title-hyphen-name-with-id` | `title-p{id}-name` | `【SING女团】……飒爽上线-p2-完整舞蹈版` |
| `title-hyphen-space-name` | `title - name` | `【SING女团】……飒爽上线 - 完整舞蹈版` |
| `title-hyphen-space-name-with-id` | `title - p{id} - name` | `【SING女团】……飒爽上线 - p2 - 完整舞蹈版` |

若希望文件名也使用该标题，可将子路径模板末段设为 `{mp_title}`，例如 `-tp "{title}/{mp_title}"`。

## 多 P 作为多版本（Emby / Jellyfin）

- 参数 `--mp-as-multiple-version`
- 配置项 `basic.mp_as_multiple_version`
- 默认值 `false`

启用后（仅对多 P 视频生效）：

- 媒体文件命名为 `title - name`（`name` 若包含 `title` 全文会先剥离），且 `title` / `name` 中的符号会替换为 `_`
- NFO 文件命名为 `title.nfo`，标题与排序标题均为总标题，同目录下多分 P 共享一份 NFO

按 Emby / Jellyfin 规则，同目录下的这些文件会被识别为同一视频的多个版本。
