...
媒体下载由 Haya 并发执行 HTTP Range 请求，本参数限制单个传输的最大并发 Worker 数量。下载器只会在有界窗口内预取；当前方块变慢或失败时，后方数据不会无限占用内存。
...
以 MiB 为单位，控制一次 HTTP Range 请求的目标块大小，不建议更改。块进入下载核心后会规范化为固定 64 KiB page；最终短 page 会按实际长度提交，不会补零。
