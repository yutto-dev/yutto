...
需要注意的是，音频保存编码 `copy` 的语义是「尽量直接拷贝」：输出容器能够直接封装源编码时不做任何转码；而当输出容器无法封装源编码时（如把 AAC 音频流装进 `.mp3` / `.flac` 这类单编码容器），我会自动转码为容器对应的编码并在日志中告知，以保证输出的可用。显式指定的非 `copy` 保存编码则总是被原样遵循。
